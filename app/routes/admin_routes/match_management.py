...
async def create_match(match: schemas.MatchCreate,sess = Annotated[AsyncSession,Depends(get_db)],current_admin: model.Player = Depends(get_current_admin_user)):
        try:
...
async def get_matches_by_tournament(tournament_id: int,sess = Annotated[AsyncSession,Depends(get_db)],current_admin: model.Player = Depends(get_current_admin_user)):
    try:
...
async def create_match_stats(match_stats: schemas.MatchStatsCreate,sess = Annotated[AsyncSession,Depends(get_db)],current_admin: model.Player = Depends(get_current_admin_user)):
        try:
            ...
            await sess.rollback()
            raise HTTPException(status_code=500, detail="Failed to create match stats")


#4. Get match statistics for a specific match:
@router.get("/matches/{match_id}/stats", response_model=schemas.MatchStatsResponse)
async def get_match_stats(match_id: int,sess = Annotated[AsyncSession,Depends(get_db)],current_admin: model.Player = Depends(get_current_admin_user)):
    try:
        result = await sess.execute(
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch match stats")
