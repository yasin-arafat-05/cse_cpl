...
async def tournament_image_upload(tounament_id:int,file: UploadFile = File(...),bgtask : BackgroundTasks = None,user = Depends(get_current_user)):
  try:
...
async def get_tounament_image_files(tounament_id:int):
    async with asyncSession() as sess:
        tournament_result = await sess.execute(
